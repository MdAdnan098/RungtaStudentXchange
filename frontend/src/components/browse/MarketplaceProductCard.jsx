...
/**
 * Fork of components/browse/ProductCard.jsx used only by the two
 * in-scope Marketplace consumers (Browse's grid, Product Details'
 * Related Listings) — kept separate from the original so Wishlist
 * and My Listings (which also render ProductCard, via WishlistCard /
 * MyListingCard) are never touched by this pass. Same props, same
 * behavior; the only real addition is a graceful fallback if the
 * cover image URL fails to load (broken/expired Cloudinary link)
 * instead of showing the browser's default broken-image icon.
 */
const MarketplaceProductCard = ({ product, isWishlisted, onToggleWishlist, showStatusBadge = false }) => {
