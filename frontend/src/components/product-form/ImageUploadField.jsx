...
import { ImagePlus, ImageOff, X, Star, UploadCloud } from "lucide-react";
...
/**
 * `images` is an array of { id, url, file } — `file` is null for an
 * existing image being kept as-is (edit mode) and a real File for a
 * newly-added one. ProductForm owns this array and decides at submit
 * time whether anything actually changed (see its comments on the
 * updateProduct all-or-nothing image limitation).
 *
 * Presentation mirrors Create Listing's image uploader (cover badge,
 * larger tiles, drag-active state) so both forms feel like the same
 * flow — same add/remove/drag-drop behavior as before, visuals only.
 */
