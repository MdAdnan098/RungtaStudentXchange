...
/**
 * Fresh Listings' own card. Intentionally a sibling of
 * components/browse/ProductCard rather than a shared component: the
 * two live on different pages (homepage vs Browse/Wishlist/Related
 * Products), and the brief for this polish pass was scoped to Fresh
 * Listings only, so this file — not the shared ProductCard — is
 * where that visual refinement lives. Same props, same behavior
 * (wishlist toggle, links to /products/:id), presentation only.
 */
const FreshListingCard = ({ product, isWishlisted, onToggleWishlist }) => {
  ...
  return (
    ...
    >
      ...
      >
        <Heart className={cn("h-4 w-4 transition-transform duration-base ease-standard", isWishlisted && "fill-danger text-danger")} aria-hidden="true" />
      </button>
      ...
      <Link to={`/products/${product._id}`} className="block aspect-[4/3] w-full overflow-hidden bg-background-subtle">
        {coverImage ? (
          <img
            ...
            alt={product.title}
            loading="lazy"
            className="h-full w-full object-contain transition-transform duration-slow ease-standard group-hover:scale-[1.04]"
          />
        ) : (
